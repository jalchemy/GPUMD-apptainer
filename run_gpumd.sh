...
if command -v docker &> /dev/null; then
    ...
    docker run --rm --privileged --runtime=nvidia  --gpus all -v $(pwd):/work \
        --mount type=bind,src=$SCRIPT_DIR/,dst=/gpumd/ \
        ...
        apptainer:1.4.4.deb13slim apptainer run --nvccli --bind /var/log/ --bind /etc/nvidia-container-runtime/ --bind /var/log/ --bind /usr/bin/ /gpumd/gpumd.sif "$COMMAND" "$@"
else
